...
class Customer(Process):
    ...
    def connect_hbase(self, params):
        ...
        self.table = self.hbase_connection.table(params["table_name"])
        self.field_name = [
            "_".join(k.decode().split(":")) for k, v in self.table.row(row).items()
        ]
        print(self.field_name)

    def connect_hive(self, params):
        self.hive_connection = hive.Connection(
            host="10.30.89.124", port=10000, database="search_engine_features"
        )
    ...
    def run(self) -> None:
        while True:
            ...
            sql = """insert into {} ({}) values {}""".format(
                self.table_name, file_name, tuple(file_name_values)
            )
