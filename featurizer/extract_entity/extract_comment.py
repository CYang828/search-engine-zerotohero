...
class ExtractComment(BaseExtract):
    ...
    def extract(self, row_data):
        ...
        comment_family = dict()
        # comment_count	评论数量
        comment.comment_count = str(one_data["comment_count"])
        # voteup_count	点赞数量
        comment.voteup_count = str(one_data["voteup_count"])
        ...
        for one_comment in one_data["comments"]:
            ...
            comment_avatar_url_list.append(
                one_comment["author"]["member"]["avatar_url"]
            )
        ...
        comment.created_time_list = json.dumps(
            {"created_time_list": comment_created_time_list}
        )
        comment.content_list = json.dumps({"content_list": comment_content_list})
        comment.avatar_url_list = json.dumps(
            {"avatar_url_list": comment_avatar_url_list}
        )
        comment.name_list = json.dumps({"name_list": comment_name_list})
        comment.url_token_list = json.dumps({"url_token_list": comment_url_token_list})
        comment.vote_count_list = json.dumps(
            {"vote_count_list": comment_vote_count_list}
        )
        return comment
