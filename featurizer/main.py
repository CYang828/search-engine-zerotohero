...
from features_utils import extract_article, hanlp_token_and_ner
...
class Process:
    ...
    def __init__(self, url='mongodb://10.30.89.124:27011/', db='zhihu_new', collection='articles', h_table_name='document_features'):
        """
    ...
    def get_data_from_mongodb(self):
        ...
        """
        return self.collec.find(no_cursor_timeout=True, batch_size=500)
    ...
    def get_document_inf(self, one_data):
        ...
        document_tokens, document_entity = hanlp_token_and_ner(clean_content)  # 使用hanlp工具进行分词和实体识别
        document_top5words = analyse.textrank(clean_content, topK=5, withWeight=False, allowPOS=('ns', 'n', 'vn', 'v'))  # 从ns地名，n名词，vn名动词， v动词这些词性中提取关键词
        document_faimly['document:tolens'] = document_tokens
    ...
    def insert_hbase(self, row_data):
        ...
        self.table.put(row_data['author:url_token'] + '_' + str(row_data['document:id']), row_data)   # 在row1行,cf:1列插入值1 table.put("row1",{"cf:1":"1"})
