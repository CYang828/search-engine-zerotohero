...
class DFAFilter:
    ...
    Use Deterministic Finite automation to reduce replacement times
    """
    ...
    def filter(self, message, repl="*"):
        ...
        while start < len(message):
            ...
            for char in message[start:]:
                if char in level:
                    step_ins += 1
                    if self.delimit not in level[char]:  # 如果没有出现结束符号delimit，继续匹配
                        level = level[char]
                    else:
                        ret.append(repl * step_ins)
                        start += step_ins - 1  # 如果不是敏感字，则跳出循环，并把下标前移一位
                        break
                else:
...
if __name__ == "__main__":
    gfw = DFAFilter()
    path = "public_store/sensitive-words/sensitive.txt"
    gfw.parse(path)
