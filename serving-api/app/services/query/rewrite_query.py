...
class RewriteQuery:
    def __init__(self):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 指定设备


        self.tokenizer = BertTokenizer.from_pretrained(
            BASE_DIR + "/query/public_store/macbert4csc-base-chinese")  # 调用tokenizer返回一个句子的input_ids（字典id）、 token_type_ids（第一个句子）、 attention_mask（mask遮罩减小误差）信息

        self.model = BertForMaskedLM.from_pretrained(BASE_DIR + "/query/public_store/macbert4csc-base-chinese")  # 初始化模型并加载参数
        self.model = self.model.to(self.device)  # 模型加载到指定设备
        # 读取相似词表
        with open(BASE_DIR + "/query/public_store/HIT-IRLab.txt", 'r', encoding='GB18030') as f:  # 注意词表的编码格式是GB18030
            lines = f.readlines()
        # 只用其中相似的词
        ...
        with open(BASE_DIR + "/query/public_store/tencent-AILab-ChineseEmbedding/tc_word_index.json", 'r') as fp:
            self.word_index = json.load(fp)
        self.tc_index = AnnoyIndex(200)
        self.tc_index.load(BASE_DIR + "/query/public_store/tencent-AILab-ChineseEmbedding/tc_index_build10.index")
        # 反向id==>word映射词表
        ...
        for word in query:
            try:  # OOV问题还用原来的词
                ...
                for i, item in enumerate(self.tc_index.get_nns_by_item(self.word_index[word], topk)):
                    results[i].append(self.reverse_word_index[item])  # self.reverse_word_index[item]用每个索引查询word
