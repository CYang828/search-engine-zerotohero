...
class ArticleSampler(BaseSampler):
    ...
    def sample(self, debug=False):
        ...
        user_click_list = [
            each for each in user_id_list if each not in set(user_no_click_list)
        ]
        ...
        item_information = pd.concat(
            (
                each.T
                for each in tqdm(temp_item_list, total=len(temp_item_list), desc="合并df")
            ),
            axis=1,
        ).T
        ...
        item_information["search_token"] = item_information["clean_token"].apply(
            lambda x: str(sample_token(eval(x)))
        )
