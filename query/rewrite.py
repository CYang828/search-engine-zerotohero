...
class RewriteQuery:
    ...
    def load_similar_words(self):
        ...
        with open(
            BASE_DIR + "/query/model_data/HIT-IRLab.txt", "r", encoding="GB18030"
        ) as f:
            lines = f.readlines()
        # 只用其中相似的词
    ...
    def load_index_word_vector(self):
        with open(
            BASE_DIR
            + "/query/model_data/tencent-AILab-ChineseEmbedding/tc_word_index.json",
            "r",
        ) as fp:
        ...
        self.tc_index.load(
            BASE_DIR
            + "/query/model_data/tencent-AILab-ChineseEmbedding/tc_index_build10.index"
        )
        # 反向id==>word映射词表
